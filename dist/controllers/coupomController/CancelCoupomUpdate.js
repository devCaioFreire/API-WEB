...
// src/controllers/coupomController/CancelCoupomUpdate.ts
var CancelCoupomUpdate_exports = {};
__export(CancelCoupomUpdate_exports, {
  UpdateCoupomStatusController: () => UpdateCoupomStatusController
});
module.exports = __toCommonJS(CancelCoupomUpdate_exports);
...
// src/services/coupomService/CancelCoupomUpdate.ts
var UpdateCoupomStatusService = class {
  execute(_0, _1) {
    return __async(this, arguments, function* ({ id, status }, token) {
      token = token.slice(7);
      const prisma = createPrismaClientFromJWT(token);
      try {
        yield prisma.pedidos_venda.update({
          where: { id },
          data: { status }
        });
...
// src/controllers/coupomController/CancelCoupomUpdate.ts
var UpdateCoupomStatusController = class {
  handle(req, res) {
    return __async(this, null, function* () {
      const { id, status } = req.body;
      const { authorization } = req.headers;
      if (authorization && typeof authorization !== "string")
        throw new ErrorResponse(400, "Token Invalid Or NotFound");
      const updateCoupomStatusService = new UpdateCoupomStatusService();
      try {
        const result = yield updateCoupomStatusService.execute({ id, status }, authorization);

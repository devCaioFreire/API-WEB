...
// src/controllers/productController/OrderController.ts
...
// src/services/productsService/OrderService.ts
var OrderService = class {
  get(token, selectors, params) {
    return __async(this, null, function* () {
      ...
      try {
        ...
        if (selectors && selectors.length > 0) {
          ...
          for (const filter of selectors) {
            ...
            if (filter.field === "dateInitial") {
              ...
              query.where["data_realizacao"] = __spreadProps(__spreadValues({}, dateCondition), { gte: new Date(filter.value) });
              continue;
            }
            if (filter.field === "dateFinal") {
              const dateCondition = query.where["data_realizacao"] || {};
              query.where["data_realizacao"] = __spreadProps(__spreadValues({}, dateCondition), { lt: new Date(filter.value) });
              query.where["data_realizacao"].lt.setDate(query.where["data_realizacao"].lt.getDate() + 1);
              continue;
            } else {
        ...
        const PedidoVenda = yield prisma.pedidos_venda.findMany({ where: query.where, skip: query.skip, take: query.take, orderBy: query.orderBy });
        return PedidoVenda;
      } catch (error) {
  ...
  getOrderByPaymentMethod(token, method) {
    return __async(this, null, function* () {
      const prisma = createPrismaClientFromJWT(token);
      try {
        const SaleOrder = yield prisma.pedidos_venda.findMany({ where: { forma_pagamento: method } });
        const listId = [];
...
var OrderController = class {
  handle(req, res) {
    return __async(this, null, function* () {
      ...
      let ParamConfig = [];
      const ParamFilter = [];
      Object.entries(params).map(([field, value]) => ParamConfig.push({ field, value }));
      ...
      if (filters != void 0 && filters != null) {
        ...
        const Order = yield orderService.get(authorization, orderService.ParamPropsFormater(ParamFilter), ParamConfig);
        if (!Order) {
  ...
  getOrderByPaymentMethod(req, res) {
    return __async(this, null, function* () {
      ...
      const Orders = yield OrderServices.getOrderByPaymentMethod(authorization, method);

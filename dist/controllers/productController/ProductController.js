...
// src/controllers/productController/ProductController.ts
...
// src/services/productsService/ProductService.ts
var ProductService = class {
  execute(ean, token) {
    return __async(this, null, function* () {
      const prisma = createPrismaClientFromJWT(token);
      try {
        const products = yield prisma.produtos.findFirst({
          where: { codEAN: ean }
        });
        return products;
      } catch (err) {
      ...
      if (filters != void 0 && filters != null) {
        for (const filter of filters) {
          ...
          json.forEach((element) => {
            ParamFilter2.push({ field: element.field, value: element.value });
          });
        }
        const productService = new ProductService();
        const product = yield productService.get(authorization, productService.ParamPropsFormater(ParamFilter2), ParamConfig);
        if (!product) {

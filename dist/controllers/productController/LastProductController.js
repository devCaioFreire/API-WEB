...
// src/controllers/productController/LastProductController.ts
var LastProductController_exports = {};
__export(LastProductController_exports, {
  getLastProductController: () => getLastProductController
});
module.exports = __toCommonJS(LastProductController_exports);
...
// src/services/productsService/LastProductService.ts
function getLastProductService(token) {
  return __async(this, null, function* () {
    const prisma = createPrismaClientFromJWT(token);
    try {
      const lastProduct = yield prisma.produtos.findFirst({
        orderBy: {
          id: "desc"
        }
      });
...
// src/controllers/productController/LastProductController.ts
var getLastProductController = class {
  handle(req, res) {
    return __async(this, null, function* () {
      try {
        ...
        if (!token || token.trim() === "") {
          throw new Error("Token inv\xE1lido ou ausente");
        }
        const nextProduct = yield getLastProductService(token);
        res.status(200).json({ nextProduct });
      } catch (error) {
        res.status(500).json({ error: "Erro ao obter o pr\xF3ximo n\xFAmero de produto" });
      }
    });
  }
};

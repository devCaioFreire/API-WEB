...
CREATE TABLE `produtos` (
    ...
    `qtdCom` DECIMAL(21, 10) NULL,
    `vlrUnCom` DECIMAL(15, 2) NULL,
    `vlrProd` DECIMAL(15, 2) NULL,
    `codEANTrib` VARCHAR(14) NULL,
    `unTrib` VARCHAR(6) NULL,
    `qtdTrib` DECIMAL(15, 4) NULL,
    `vlrUnTrib` DECIMAL(21, 10) NULL,
...
-- AddForeignKey
ALTER TABLE `notafiscalitem` ADD CONSTRAINT `notafiscalitem_ibfk_1` FOREIGN KEY (`notafiscal_id`) REFERENCES `notafiscal`(`id`) ON DELETE RESTRICT ON UPDATE RESTRICT;

-- AddForeignKey
ALTER TABLE `notafiscalitem` ADD CONSTRAINT `notafiscalitem_ibfk_2` FOREIGN KEY (`produto_id`) REFERENCES `produtos`(`id`) ON DELETE RESTRICT ON UPDATE RESTRICT;
